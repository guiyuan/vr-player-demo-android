...
public class TexturePack {
	 ...
	 * @param forceSquareTexture 
	 * 
	 * @return created image
	 */
	private Bitmap packImage(boolean forceSquareTexture) {
		checkPacked();
		...
		loop: while (!fitAll) {
			...
			if (area < totalArea) {
				nextSize(size, forceSquareTexture);
				if (DEBUG)
	 ...
	 * @see #ALPHA_DONT_USE
	 */
	public Texture pack(boolean useAlpha, boolean forceSquareTexture) {
		if (texture != null)
			return texture;

		Bitmap image = packImage(forceSquareTexture);
	...
	private void nextSize(Point size, boolean forceSquareTexture) {
		if (forceSquareTexture) {
			size.x <<= 1;
			size.y <<= 1;
		} else {
